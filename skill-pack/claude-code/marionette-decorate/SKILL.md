...
- The `Program.cs` calls `Marionette.Runtime.MarionetteHost.RunAsync(args, Marionette.Generated.GeneratedManifest.Roots, adapter)` to bring up the MCP host. There is a second entry point `MarionetteHost.RunAsyncSourceGenSafe(...)` with the same parameter shape but without `[RequiresUnreferencedCode]` / `[RequiresDynamicCode]` annotations — use it when the user (a) does not invoke `raise_event` from MCP clients AND (b) every `[McpEvent]` args type, `[McpObservable]` value type, and `[McpCallable]` return + parameter type stays within the source-gen-eligible shape set (primitives, plain user records, `Nullable<T>`, enums, `T[]`, `List<T>`, every standard collection interface, `Dictionary` / `IDictionary` / `IReadOnlyDictionary` across STJ-supported key types, plus user / concurrent collection types matched via Phase-11's interface-fallback walker). The strict entry point is the right default for adopters who AOT-publish and stick to `[McpCallable]` + `simulate_input` for app driving — see `docs/stripping.md` for the full contract.
...
- TFM: WPF requires `net10.0-windows`. Avalonia adopters should use `net10.0` (NOT `net10.0-windows`) because Avalonia is cross-platform. WinUI 3 requires `net10.0-windows10.0.<sdk>.0` (e.g. `net10.0-windows10.0.19041.0` for Windows App SDK 2.x) plus `<UseWinUI>true</UseWinUI>` and `<WindowsPackageType>None</WindowsPackageType>` for unpackaged deployment — note that unpackaged WinAppSDK 2.x EXEs require `Microsoft.WindowsAppRuntime 2.x` to be installed on the target machine (see docs/getting-started.md Prerequisites). MAUI single-targets `net10.0-windows10.0.19041.0` for Phase 4.1; multi-target with Android / iOS / MacCatalyst requires those platform toolchains and is a Phase-6 follow-up.
